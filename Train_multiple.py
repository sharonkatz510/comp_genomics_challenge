...
for idx, i in enumerate(range(100)):
    lr = Regression.RidgeReg(filename="Known_set_Bacillus.xlsx", test_size=0.5, drop_wins=True, normalize=True)
...
# only_best = Regression.RidgeReg(data=lr.X[list(best_features)], label=lr.Y, test_size=0.5)
